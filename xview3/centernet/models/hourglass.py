...
from ..constants import CENTERNET_OUTPUT_CLASS_MAP, CENTERNET_OUTPUT_OBJECTNESS_MAP

__all__ = ["get_large_hourglass_net"]
...
class residual(nn.Module):
    def __init__(self, k, inp_dim, out_dim, stride=1, with_bn=True):
        ...
        # This reduces model params number twice
        self.conv1 = DepthwiseSeparableConv2d(inp_dim, out_dim, (3, 3), padding=(1, 1), stride=(stride, stride), bias=False)
        ...
        self.skip = (
            nn.Sequential(nn.Conv2d(inp_dim, out_dim, (1, 1), stride=(stride, stride), bias=False), nn.BatchNorm2d(out_dim))
...
class exkp(nn.Module):
    def __init__(
        ...
        self.inters_ = nn.ModuleList(
            [nn.Sequential(nn.Conv2d(curr_dim, curr_dim, (1, 1), bias=False), nn.BatchNorm2d(curr_dim)) for _ in range(nstack - 1)]
        )
        self.cnvs_ = nn.ModuleList(
            [nn.Sequential(nn.Conv2d(cnv_dim, curr_dim, (1, 1), bias=False), nn.BatchNorm2d(curr_dim)) for _ in range(nstack - 1)]
        )

        ## keypoint heatmaps
        for head in heads.keys():
            if CENTERNET_OUTPUT_CLASS_MAP in head:
                module = nn.ModuleList([make_heat_layer(cnv_dim, curr_dim, heads[head]) for _ in range(nstack)])
                self.__setattr__(head, module)
                for heat in self.__getattr__(head):
                    heat[-1].bias.data.fill_(-2.19)
            elif CENTERNET_OUTPUT_OBJECTNESS_MAP in head:
                module = nn.ModuleList([make_heat_layer(cnv_dim, curr_dim, heads[head]) for _ in range(nstack)])
                self.__setattr__(head, module)
                for heat in self.__getattr__(head):
                    heat[-1].bias.data.fill_(-2.19)
...
class HourglassNet(exkp):
    def __init__(self, config):
        ...
        self.box_coder = instantiate(config["box_coder"], output_stride=4)

    def forward(self, image):
        outputs = super(HourglassNet, self).forward(image)
        return outputs[-1]


def get_large_hourglass_net(config):
    model = HourglassNet(config)
    return model
